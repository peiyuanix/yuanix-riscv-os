#!/usr/bin/python3
# qemu virt debug mouse example: by Brent Hartshorn
# options: --mouse --no-uart --no-threads
# args: A.jpg B.jpg foo. bar.
...
def gen_images_header(images):
	return [ o['data'] for o in images]
...
def gen_firmware_no_threads(images, strings):
	out = [
		'void firmware_main(){'
		'	uart_init();',
 		'	uart_print("[firmware_main no threads]\\n");',
 		'	while (1) {',
	]
	for i, img in enumerate(images):
		#out.append(img['redraw'])
		out.append('uart_print("%s");' % strings[i])
...
def meme(images, strings):
	...
	if '--no-threads' in sys.argv:
		out += gen_images_header(images)
		out += gen_firmware_no_threads(images, strings)
	else:
		out += gen_proc_header(images) + gen_procs(images,strings) + [ INTERRUPTS ]
		out += gen_firmware(images, strings)
	c = '\n'.join(out)
